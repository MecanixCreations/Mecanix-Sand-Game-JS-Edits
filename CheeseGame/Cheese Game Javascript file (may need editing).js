
    /* Cheese Game ;Mecanix Creations 2024*/ ! function(n, r) {
    "object" == typeof exports && "undefined" != typeof module ? r(exports) : "function" == typeof define && define.amd ? define(["exports"], r) : r((n = "undefined" != typeof globalThis ? globalThis : n || self).SandSaga = {})
}(this, (function(n) {
    "use strict";
      var img$1 = "data:image/png;base64,";
      var img$2 = "data:image/png;base64,"
      var img$3 ="data:image/png;base64,"
      var img$4 ="data:image/png;base64,"
      var img$a = "data:image/png;base64,";
      //houses
      var img$10 = "data:image/png;base64," 
      var img$11 = "data:image/png;base64," 
      var img$12 = "data:image/png;base64," 
      var img$13 = "data:image/png;base64," 
      var img$56 = "data:image/png;base64,"
      var img$57 = "data:image/png;base64,"
        //Tree
        var img$18 = "data:image/png;base64,"
        var img$19 = "data:image/png;base64,"
        var img$20 = "data:image/png;base64,"
        var img$21 = "data:image/png;base64,"

      //cheeseunguses
    var img$14 = "data:image/png;base64,"
    var img$15 = "data:image/png;base64,"
    var img$16 = "data:image/png;base64,"
    var img$17 = "data:image/png;base64,"
    var img$54 = "data:image/png;base64,"
    var img$55 = "data:image/png;base64,"
        //BOB

        var img$22 = "data:image/png;base64,"
//Pallet
var img$23 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADNJREFUKFNj/LNO7j8DFLAEPWKkhM94q48ZbhjMUHJpxh+LIIZxxP1lxGYITJ4YNVQ1DADtlSpufcvnMwAAAABJRU5ErkJgggAA"
var img$24 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADNJREFUKFNj/DNZ7j8DFLDkPmKkhM94q4QZbhjMUHJpxh8tEMM4av4yYjMEJk+MGqoaBgCoYyn9//f7iAAAAABJRU5ErkJgggAA"
var img$25 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADNJREFUKFNj/FMs958BClh6HzFSwme8FcoMNwxmKLk0449EiGEc8/8yYjMEJk+MGqoaBgBWfyl4KA3t3AAAAABJRU5ErkJgggAA"
var img$26 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADRJREFUKFNj3KWr+58BCtwuX2akhM/YoagINwxmKLk040J1dbBh8TdvMmIzBCZPjBqqGgYANtgnvaReswwAAAAASUVORK5CYIIA"
var img$27 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADRJREFUKFNj/Pjs0n8GKOCX0mOkhM/46uZuuGEwQ8mlGR+fWQI2TNYkhhGbITB5YtRQ1TAAPPItzpIfl4cAAAAASUVORK5CYIIA"
var img$52 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAADxJREFUGJVjtE2d8J8BCg7PLmCkhM9oFtUI51AKWH59+8TAwMDAcGFDLyM2BQYBxXDLCKlhoparGBgYGABz5x8sWd2Q1gAAAABJRU5ErkJggg=="
var img$53 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABMAAAAECAYAAABsgwTvAAAAAXNSR0IArs4c6QAAAD1JREFUGJVjtLCw+M8ABSdOnGCkhM+oqqoK51AKWH79+sXAwMDA8PDhQ0ZsCuTl5eGWEVLDRC1XMTAwMAAAanAeDGekaN0AAAAASUVORK5CYII="  

//rocks_small
var img$28 = "data:image/png;base64,"
var img$29 = "data:image/png;base64,"
var img$30 = "data:image/png;base64,"
var img$31 = "data:image/png;base64,"
var img$32 = "data:image/png;base64,"
var img$33 = "data:image/png;base64,"
var img$34 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAARBAMAAAAvapGNAAAAAXNSR0IArs4c6QAAAA9QTFRFAAAAoScnui0tzzc31VBQe/kLSwAAAAF0Uk5TAEDm2GYAAABHSURBVAgdtcEBFYAgEETB/6AAZwOXAnp7DbB/JiWEM2zh52Jr2gagaac0oE27Ujqh22VLGnTXckoKqOXUJ2D5kIKt3zH40ws1qgmlpQS1LAAAAABJRU5ErkJggg=="
var img$35 = "data:image/png;base64,"
var img$36 = "data:image/png;base64,"
var img$37 = "data:image/png;base64,"
var img$38 = "data:image/png;base64,"
var img$39 = "data:image/png;base64,"
var img$40 = "data:image/png;base64,"
var img$41 = "data:image/png;base64,"
var img$42 = "data:image/png;base64,"
//CHeese_tiles
var img$43 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAIBAMAAAA2IaO4AAAAAXNSR0IArs4c6QAAAA9QTFRFiCEhoScnui0tzzc31VBQxHQ8qQAAAB9JREFUCNdjcHExNmZwVlJSZDDCQggpKQowCAoKMgAAa9YEi/qIH8kAAAAASUVORK5CYII="
var img$44 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEZJREFUKFNj/DMz6j8DMnj7CIXLCFYAEkSTAKn68+4pA+OfNhuIAiyAXgp+JDL/ZxGSRnEByG4QePSFgYERpACbA0GSIAAA5Mg27//5KyQAAAAASUVORK5CYIIA"
var img$45 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEhJREFUKFNj/LMh6j8DMvj4CIXLCFYAEvyEKgFS9efjUwbGPwtt/mOTpKeCHy3M/1n4pVFcDnIcCDz6wMDACFKAIgvlgCRBAAB5mzf3LEQZaQAAAABJRU5ErkJgggAA"
var img$46 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEZJREFUKFNj/HMk6j8DMvj6CIXLCFYAEkSTAKn68+UpA+OfnTYQBVgAvRT8WMT8n4VHGsUFILtB4NEbBgZGkAJsDgRJggAA+bc42SAn284AAAAASUVORK5CYIIA"
var img$47 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEVJREFUKFNj/P///38GJPDzyytkLgMjSAFI8OfnlygSIA5IjPHH55f/sUnSU8Grm7v/s/OKozgQ5iawI0EKMJwP9QFIHAAcC0jxdsT6/QAAAABJRU5ErkJgggAA"
var img$48 = "data:image/png;base64,"
var img$49 = "data:image/png;base64,"
var img$50 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEtJREFUGJVjTG5e9p8BCbz9+BWZy8ACE0SXYGBgYPj59SMDEy5JGGDCKUM9Bc+ePcPquJ9fPzL8+vYJ4oufXz9iKPr17RMDAwMDAwA25CSwnErOGgAAAABJRU5ErkJggg=="
var img$51 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAEpJREFUGJWtj7kNwDAMA89GNtAM2ky7q+ICapwiMGA4cZfr+BRki4jBQmaukmuaktiRRD+Fk35Mfis0dx9m9hoHUFXPi6+RVQXADaMDH8MVntqkAAAAAElFTkSuQmCC"
//xtra blue & black stuff
var img$58 = "data:image/png;base64,"
var img$59 = "data:image/png;base64,"
    var img$j = "data:image/png;base64,";

      var img$i = "data:image/png;base64,";

      var img$h = "data:image/png;base64,";

      var img$g = "data:image/png;base64,";

      var img$f = "data:image/png;base64,";

      var img$e = "data:image/png;base64,";

      var img$b = "data:image/png;base64,";

      var img$9 = "data:image/png;base64,";

      var img$8 = "data:image/png;base64,";

      var img$7 = "data:image/png;base64,";

      var img$6 = "data:image/png;base64,";

      var img$5 = "data:image/png;base64,";

      var img$c = "data:image/png;base64,";
      //big_block_cheese
      var img$m = "data:image/png;base64,"
      //big_cheese
      var img$n = "data:image/png;base64,"
      //big_pasta
      var img$o = "data:image/png;base64,"
      //cheese_med
      var img$p = "data:image/png;base64,"
      //cheese_small
      var img$q = "data:image/png;base64,"
      //cheese
      var img$r = "data:image/png;base64," 
      //err_cheese
      var img$s = "data:image/png;base64,"
      //house_big
      var img$t = "data:image/png;base64,"
      //house_small
      var img$u = "data:image/png;base64,"
      //parmesan_pile
      var img$v = "data:image/png;base64,"
      //parmesan
      var img$w = "data:image/png;base64,"
      //pasta_long
      var img$x = "data:image/png;base64,"
      //pasta
      var img$y = "data:image/png;base64,"
      //cheeseungus
      var img$z = "data:image/png;base64,"
    class r {
        static #n = "/api/user/";
        static sendCompleted(n, t, e, o) {
            const a = new FormData;
            a.append("scenario", n);
            const d = r.#r(t);
            if (a.append("metadata", JSON.stringify(d)), !0 === o) {
                const n = r.#t(t);
                null !== n && a.append("data", n)
            }
            const s = fetch(r.#n + "scenario-completed", {
                method: "POST",
                body: a,
                headers: {
                    Accept: "application/json"
                }
            });
            void 0 !== e && s.then((n => n.json())).then((n => {
                "number" == typeof n.id && e(n.id)
            }))
        }
        static #e = 0;
        static #o = 3;
        static #a = 0;
        static #d = 3;
        static sendErrorReport(n, t, e, o, a) {
            if ("rendering-warning" === e) return;
            if ("user" === e) {
                if (!(r.#e < r.#o)) return;
                r.#e++
            } else {
                if (!(r.#a < r.#d)) return;
                r.#a++
            }
            const d = new FormData;
            d.append("scenario", n), d.append("location", e), d.append("message", o);
            const s = r.#r(t);
            if (d.append("metadata", JSON.stringify(s)), !0 === a) {
                const n = r.#t(t);
                null !== n && d.append("data", n)
            }
            fetch(r.#n + "report", {
                method: "POST",
                body: d
            })
        }
        static sendUpdate(n, t) {
            const e = new FormData;
            e.append("scenario", n), fetch(r.#n + "update", {
                method: "POST",
                body: e
            })
        }
        static #r(n) {
            const r = n.getSandGame();
            return {
                width: null !== r ? r.getWidth() : null,
                height: null !== r ? r.getHeight() : null,
                userAgent: navigator.userAgent
            }
        }
        static #t(n) {
            try {
                const r = n.createSnapshot();
                if (null === r) return null;
                const t = window.SandGameJS.Resources.createResourceFromSnapshot(r);
                return new Blob([t])
            } catch (n) {
                return console.warn(n), null
            }
        }
    }
    class t {
        static init(n, t, e) {
            let o = {};
            Object.assign(o, t), Object.assign(o, {
                disableSizeChange: !1,
                disableSceneSelection: !1
            }), Object.assign(o, e), o.errorReporter = (n, e, o) => {
                try {
                    r.sendErrorReport(t.scenario, o, n, e, !0)
                } catch (n) {}
            };
            const a = window.SandGameJS.init(n, o);
            let d = 0;
            const s = n => {
                    n.scenario().addOnStatusCompleted((() => {
                        if (void 0 !== t.onCompleted) try {
                            t.onCompleted()
                        } catch (n) {}
                        try {
                            r.sendCompleted(t.scenario, a, t.onAccepted, t.includeSnapshot)
                        } catch (n) {}
                    })), n.addOnProcessed((() => {
                        d++, 1e4 === d && n.scenario().setCompleted()
                    }))
                },
                c = a.getSandGame();
            return null !== c && s(c), a.addOnInitialized(s), setInterval((() => r.sendUpdate(t.scenario, a)), 6e4), a
        }
    }
    class e {
        static widthFraction(n, r) {
            return Math.trunc(n.getWidth() / r)
        }
        static heightFraction(n, r) {
            return Math.trunc(n.getHeight() / r)
        }
        static mapPositionFracWH(n, r, t, e) {
            let a, d;
            if (void 0 !== e)
                if (1 === e) {
                    const r = Math.min(n.getWidth(), n.getHeight());
                    a = r, d = r
                } else e > 1 ? (a = Math.min(n.getWidth(), n.getHeight() * e), d = a / e) : (d = Math.min(n.getHeight(), n.getWidth() / e), a = d * e);
            const s = void 0 === a ? n.getWidth() : Math.min(a, n.getWidth()),
                c = void 0 === d ? n.getHeight() : Math.min(d, n.getHeight()),
                l = void 0 === a ? 0 : Math.trunc((n.getWidth() - s) / 2),
                A = void 0 === d ? 0 : Math.trunc((n.getHeight() - c) / 2);
            return new o(l, A, s / r, c / t)
        }
    }
    class o {
        #s;
        #c;
        

        #l;
        #A;
        constructor(n, r, t, e) {
            this.#s = n, this.#c = r, this.#l = t, this.#A = e
        }
        x(n) {
            return Math.trunc(this.#s + n * this.#l)
        }
        y(n) {
            return Math.trunc(this.#c + n * this.#A)
        }
        w(n) {
            return Math.trunc(n * this.#l)
        }
        h(n) {
            return Math.trunc(n * this.#A)
        }
    }
    class a {
        static addContent(n, r) {
            if (null === r);
            else if ("string" == typeof r) n.textContent = r;
            else if (r instanceof Node) n.appendChild(r);
            else {
                if (!Array.isArray(r)) throw "Content type not supported: " + typeof r;
                for (const t of r)
                    if (t instanceof Node) n.appendChild(t);
                    else if ("string" == typeof t) n.insertAdjacentText("beforeend", t);
                else if (null !== t) throw "Content type not supported: " + typeof t
            }
        }
        static setContent(n, r) {
            n.innerHTML = "", a.addContent(n, r)
        }
        static putAttributes(n, r) {
            for (const t in r) {
                const e = r[t],
                    o = typeof e;
                if ("string" === o || "boolean" === o || "number" === o) n.setAttribute(t, e);
                else if ("style" === t && "object" === o) Object.assign(n.style, e);
                else if (null !== e) throw "Unsupported attribute type: " + typeof e
            }
        }
        static create(n) {
            const r = document.createElement("template");
            return r.innerHTML = n.trim(), r.content.firstElementChild
        }
        static element(n, r = null, t = null) {
            const e = document.createElement(n);
            return a.putAttributes(e, r), a.addContent(e, t), e
        }
        static div(n = null, r = null) {
            return a.element("div", n, r)
        }
        static par(n = null, r = null) {
            return a.element("p", n, r)
        }
        static span(n = null, r = null) {
            return a.element("span", r, n)
        }
    }
    const d = window.SandGameJS.BrushDefs,
        s = window.SandGameJS.Brushes,
        c = window.SandGameJS.ToolDefs,
        l = window.SandGameJS.Tools,
        Q = window.SandGameJS.TemplateDefs,
        A = window.SandGameJS.PredicateDefs;    
    n.init = function(n, r) {
        function o(n, r, t, e, o,u,y,x,z) {
            let a = s.colorRandomize(x, s.colorPaletteRandom(r, u));
            return l.roundBrushTool(a, z, c.SAND.getInfo().derive({
                codeName: n,
                displayName: y,
                badgeStyle: {
                    backgroundColor: `rgb(${t},${e},${o})`
                }
            }))
        }
        const i = o("blue_cheese", "217,205,181\r\n201,195,165\r\n201,181,148\r\n62,102,141\r\n154,167,180", 62,102,141, d.SAND, "Blue Cheese",4,c.DEFAULT_SIZE),
            u = o("red_cheese", "136,33,33\r\n161,39,39\r\n186,45,45\r\n207,55,55\r\n213,80,80\r\n217,181,181\r\n201,148,148\r\n201,165,165", 136,33,33, d.SAND, "Red Cheese",4,c.DEFAULT_SIZE),
            h = o("cheddar_jack_cheese", "229,172,26\r\n229,156,26\r\n238,211,172\r\n253,230,191", 229,172,26, d.GRAVEL, "Cheddar Jack",7,c.DEFAULT_SIZE),
            f = o("test_cheddar_cheese", "249,192,46\r\n249,176,46", 249,192,46, d.SOIL, "(old) Grated Cheddar Cheese",8,c.DEFAULT_SIZE),
            g = o("parmesian_cheese", "228,211,162\r\n233,220,181\r\n239,229,199\r\n244,237,218\r\n250,246,236", 228,211,162, d.SAND, "Parmesian Cheese",2,c.DEFAULT_SIZE),
            F = o("grass_mac", "218,116,3\r\n238,136,23\r\n228,126,13", 218,116,3, d.GRASS, "Mac n Cheese",2,c.DEFAULT_SIZE),
            U = o("tree_pasta", "221,180,117", 221,180,117, d.TREE, "Pasta Tree",9,1),
            J = o("tree_spagetti", "251,219,101", 251,219,101, d.TREE_LEAF_LIGHTER, "Spagetti Leaf",9,c.DEFAULT_SIZE),
            G = o("pasta_natural", "221,180,117\r\n227,203,156", 221,180,117, d.TREE_WOOD, "Hard Pasta",6,c.DEFAULT_SIZE),
            p = o("knife_cut", "221,180,117\r\n227,203,156", 255,186,119, d.AIR, "Knife Cut",6,2),
            H = o("velveta ", "255, 166, 0", 255, 166, 0, d.WATER, "Cheese Sauce",1,c.DEFAULT_SIZE),
            I = o("thermite_cheese", "242, 124, 66", 242, 124, 66, d.THERMITE, "Quick Igniting Cheese",15,c.DEFAULT_SIZE),
            FF = o("cheese_ignite", "255,186,119", 255,186,119, d.FIRE, "Cheese Based Igniter",15,16),
            TT = o("eat", "255,255,255", 221,180,117, d.AIR, "Eat",15,c.DEFAULT_SIZE),
            BASIC_SHREDDED = o("cheddar_cheese", "218,116,3,\r\n248,132,3,\r\n252,147,30,\r\n252,161,60,\r\n252,176,90,\r\n", 252,147,30, d.SOIL, "Cheese",4,c.DEFAULT_SIZE),
            LIGHT_SHREDDED = o("light_cheddar_cheese", "218,142,3,\r\n248,162,3,\r\n252,174,30,\r\n252,185,60,\r\n252,196,90,\r\n", 252,174,30, d.SAND, "Light Cheese",4,c.DEFAULT_SIZE),
            SHARP_SHREDDED = o("sharp_cheddar_cheese", "218,85,3,\r\n248,97,3,\r\n252,115,30,\r\n252,134,60,\r\n252,153,90,\r\n", 252,115,30, d.SAND, "Sharp Cheese",4,c.DEFAULT_SIZE),
            RED_SHREDDED = o("red_cheddar_cheese", "136,33,33,\r\n161,39,39,\r\n186,45,45,\r\n207,55,55,\r\n213,80,80,\r\n", 186,45,45, d.SAND, "Red Cheese",4,c.DEFAULT_SIZE),
            WHITE_SHREDDED = o("white_cheddar_cheese", "227,204,164,\r\n234,217,187,\r\n241,230,210,\r\n248,243,233,\r\n255,255,255,\r\n", 241,230,210, d.SAND, "White Cheese",4,c.DEFAULT_SIZE),
            GLOB_SHREDDED = o("glob_cheddar_cheese", "218,116,3,\r\n248,132,3,\r\n252,147,30,\r\n252,161,60,\r\n252,176,90,\r\n218,142,3,\r\n248,162,3,\r\n252,174,30,\r\n252,185,60,\r\n252,196,90,\r\n218,85,3,\r\n248,97,3,\r\n252,115,30,\r\n252,134,60,\r\n252,153,90,\r\n136,33,33,\r\n161,39,39,\r\n186,45,45,\r\n207,55,55,\r\n213,80,80,\r\n227,204,164,\r\n234,217,187,\r\n241,230,210,\r\n248,243,233,\r\n255,255,255,\r\n48,80,115\r\n54,90,129\r\n61,101,144\r\n80,116,155\r\n99,131,166\r\n31,31,31\r\n37,37,37\r\n56,56,56\r\n75,75,75\r\n100,100,100\r\n", 100,100,100, d.GRAVEL, "Conglomerate Cheese",4,c.DEFAULT_SIZE),
            BLUE_SHREDDED = o("blue_cheddar_cheese", "48,80,115\r\n54,90,129\r\n61,101,144\r\n80,116,155\r\n99,131,166\r\n", 61,101,144, d.GRAVEL, "Blue Cheese",4,c.DEFAULT_SIZE),
            BLACK_SHREDDED = o("black_cheddar_cheese", "31,31,31\r\n37,37,37\r\n56,56,56\r\n75,75,75\r\n100,100,100\r\n", 56,56,56, d.SOIL, "Black Cheese",4,c.DEFAULT_SIZE),
   
   
            VV = l.roundBrushTool(s.colorRandomize(4, s.colorTexture(img$a, d.WALL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheese_brick",
                displayName: "Cheese Bricks",
                badgeStyle: {
                    backgroundColor: "#fe9320"
                }
            })),
            //Cheese Colour Tiles
            RED_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$43, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "red_cheese_tile",
                displayName: "Red Cheese tiles",
                badgeStyle: {
                    backgroundColor: "rgb(186,45,45)"
                }
            })),
            SHARP_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$44, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "sharp_cheese_tile",
                displayName: "Sharp Cheese tiles",
                badgeStyle: {
                    backgroundColor: "rgb(252,115,30)"
                }
            })),
            BASIC_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$45, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheddar_cheese_tile",
                displayName: "Cheddar Cheese tiles",
                badgeStyle: {
                    backgroundColor: "rgb(252,147,30)"
                }
            })),
            LIGHT_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$46, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "light_cheese_tile",
                displayName: "Light Cheese tiles",
                badgeStyle: {
                    backgroundColor: "rgb(252,174,30)"
                }
            })),
            WHITE_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$47, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "white_cheese_tile",
                displayName: "White Cheese tiles",
                badgeStyle: {
                    backgroundColor: "rgb(241,230,210)"
                }
            })),
            GLOB_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$48, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheese_tile",
                displayName: "Conglomerate Cheese tiles",
                badgeStyle: {
                    backgroundColor: "#fe9320"
                }
            })),
            GLOB_SOLID = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$49, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheese_mega",
                displayName: "Conglomerate Cheese",
                badgeStyle: {
                    backgroundColor: "#fe9320"
                }
            })),
            BLUE_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$50, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheese_tile_blue",
                displayName: "Blue Cheese Tiles",
                badgeStyle: {
                    backgroundColor: "#3d6590"
                }
            })),
            BLACK_TILED = l.roundBrushToolForSolidBody(s.colorRandomize(4, s.colorTexture(img$51, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheese_tile_black",
                displayName: "Black Cheese Tiles",
                badgeStyle: {
                    backgroundColor: "#383838"
                }
            })),
            //End TIles
            EE = l.roundBrushToolForSolidBody(s.colorRandomize(2, s.colorTexture(img$3, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "red_cheese",
                displayName: "Pesto Waxed Cheese",
                badgeStyle: {
                    backgroundColor: "#b92e2d"
                }
            })),
            AA = l.roundBrushToolForSolidBody(s.colorRandomize(2, s.colorTexture(img$1, d.METAL)), c.DEFAULT_SIZE, c.METAL.getInfo().derive({
                codeName: "cheddar_cheese_block",
                displayName: "Cheddar Cheese",
                badgeStyle: {
                    backgroundColor: "#fc921f"
                }
            })),
            CC = l.roundBrushToolForSolidBody(s.colorRandomize(5, s.colorTexture(img$e, d.TREE_WOOD)), c.DEFAULT_SIZE, c.ROCK.getInfo().derive({
                codeName: "pasta_noodle",
                displayName: "Pasta Noodles",
                badgeStyle: {
                    backgroundColor: "#e1cf9d"
                }
            })),
            //biggest boi in existance
            RR = l.templateSelectionTool([
              {
                  info: {
                      displayName: "Cheese House",
                      category: "template",
                        icon: {
                            imageData: img$t
                     }
                 },
                    action: {
                    type: "random-template",
                     actions: [{
                        type: "image-template",
                        imageData: img$t,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$10,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$11,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$12,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$13,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$56,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     },{
                        type: "image-template",
                        imageData: img$57,
                        brush: "metal",
                        threshold: 1,
                        randomFlipHorizontally: !0

                     }]
                    }
                },{
                  info: {
                      displayName: "Small Cheese House",
                      category: "template",
                        icon: {
                            imageData: img$u
                     }
                 },
                    action: {
                        type: "image-template",
                        imageData: img$u,
                        brush: "metal",
                        threshold: 50,
                        randomFlipHorizontally: !0
                    }
                },{
                  info: {
                      displayName: "bob",
                      category: "template",
                        icon: {
                            imageData: img$22
                     }
                 },
                    action: {
                        type: "image-template",
                        imageData: img$22,
                        brush: "metal",
                        threshold: 50,
                        randomFlipHorizontally: !0
                    }
                },{
                  info: {
                      displayName: "Cheese Blocks",
                      category: "template",
                        icon: {
                            imageData: img$8
                     }
                 },
                    action: {
                 type: "random-template",
                actions: [{
                      type: "image-template",
                      imageData: img$8,
                      brush: "metal",
                      threshold: 1,
                      randomFlipHorizontally: !0
                  },
                  {
                     type: "image-template",
                     imageData: img$7,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$6,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$5,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$c,
                      brush: "metal",
                    threshold: 0 ,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$58,
                      brush: "metal",
                    threshold: 0 ,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$59,
                      brush: "metal",
                    threshold: 0 ,
                    randomFlipHorizontally: !0
                      }
                    ]
                    }
                },
                //BIG BOI aka. ROCK_SM
                {
                  info: {
                      displayName: "Cheese Rock Small",
                      category: "template",
                        icon: {
                            imageData: img$28
                     }
                 },
                    action: {
                 type: "random-template",
                actions: [{
                      type: "image-template",
                      imageData: img$28,
                      brush: "rock",
                      threshold: 1,
                      randomFlipHorizontally: !0
                  },
                  {
                     type: "image-template",
                     imageData: img$29,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$30,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$31,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$32,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$33,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$34,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$35,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$36,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$37,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$38,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$39,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$40,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$41,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$42,
                      brush: "rock",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      }
                    ]
                    }
                },
                //end of BIG ROK THING
                {
                  info: {
                      displayName: "Cheese Tree",
                      category: "template",
                        icon: {
                            imageData: img$18
                     }
                 },
                    action: {
                 type: "random-template",
                actions: [{
                      type: "image-template",
                      imageData: img$18,
                      brush: "wood",
                      threshold: 1,
                      randomFlipHorizontally: !0
                  },
                  {
                     type: "image-template",
                     imageData: img$19,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$20,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$21,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      }
                    ]
                    }
                },{
                  info: {
                      displayName: "Cheese Pallet",
                      category: "template",
                        icon: {
                            imageData: img$23
                     }
                 },
                    action: {
                 type: "random-template",
                actions: [{
                      type: "image-template",
                      imageData: img$23,
                      brush: "wood",
                      threshold: 1,
                      randomFlipHorizontally: !0
                  },
                  {
                     type: "image-template",
                     imageData: img$24,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$25,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$26,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$27,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$52,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$53,
                      brush: "wood",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      }
                    ]
                    }
                },{
                  info: {
                      displayName: "Cheeseungus",
                      category: "template",
                        icon: {
                            imageData: img$z
                     }
                 },
                    action: {
                 type: "random-template",
                actions: [{
                      type: "image-template",
                      imageData: img$z,
                      brush: "metal",
                      threshold: 1,
                      randomFlipHorizontally: !0
                  },
                  {
                     type: "image-template",
                     imageData: img$14,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$15,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$16,
                      brush: "metal",
                    threshold: 1,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$17,
                      brush: "metal",
                    threshold: 1 ,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$54,
                      brush: "metal",
                    threshold: 1 ,
                    randomFlipHorizontally: !0
                      },
                  {
                     type: "image-template",
                     imageData: img$55,
                      brush: "metal",
                    threshold: 1 ,
                    randomFlipHorizontally: !0
                      }
                    ]
                    }
                }
                
            ], c.ROCK_TEMPLATES.getInfo().derive({
             codeName: 'cheese_building_resources',
               displayName: 'Templates',
                badgeStyle: {
                    backgroundColor: `rgb(127, 46, 0)`,
                    }
            })),

            j = {
                tools: [TT, c.MOVE, c.FLIP_VERTICALLY,u,i,h,f,g,F,U,J,G,p,H,I,VV,CC,EE,AA,FF,RR,BASIC_SHREDDED,BASIC_TILED,LIGHT_SHREDDED,LIGHT_TILED,SHARP_SHREDDED,SHARP_TILED,RED_SHREDDED,RED_TILED,WHITE_SHREDDED,WHITE_TILED,BLUE_SHREDDED,BLUE_TILED,BLACK_SHREDDED,BLACK_TILED,GLOB_SHREDDED,GLOB_SOLID,GLOB_TILED,c.METAL,c.FIRE, c.METEOR,c.EFFECT_TEMP_MINUS,c.EFFECT_TEMP_PLUS,c.EFFECT_TEMP_PLUS2 /*c.SAND,c.ERASE_SLASH,c.FIRE_SLASH,c.HOT_SLASH,c.COLD_SLASH, c.WATER,  c.SOIL, c.GRAVEL, c.COAL,   c.ROCK_TEMPLATES, c.TEMPLATES, c.ROCK,c.TAR, c.WOOD,c.WOOD_1,c.LEAF,c.LEAF_1,c.GRASSPLACE, c.GLASS,c.METAL, c.STEEL, c.BRASS, c.GLASS_MOLTEN,c.METAL_MOLTEN,c.STEEL_MOLTEN, c.BRASS_MOLTEN,c.LAVA,*/ ],
                brushes: {wall: s.colorRandomize(5, s.colorTexture(img$a, d.WALL)),tree_wood: s.colorRandomize(5, s.colorTexture(img$e, d.WALL)), tree_wood_dark: s.colorRandomize(5, s.colorTexture(img$e, d.TREE_WOOD_DARK)), tree_leaf: s.colorRandomize(9, s.colorPaletteRandom("251,219,101", d.TREE_LEAF_LIGHTER)), tree_leaf_dark: s.colorRandomize(9, s.colorPaletteRandom("251,219,101", d.TREE_LEAF_DARKER)),sand: s.colorRandomize(5, s.colorPaletteRandom("228,211,162\r\n233,220,181\r\n239,229,199\r\n244,237,218\r\n250,246,236", d.SAND)),soil: s.colorRandomize(4, s.colorPaletteRandom("218,116,3,\r\n248,132,3,\r\n252,147,30,\r\n252,161,60,\r\n252,176,90,\r\n", d.SOIL)),gravel: s.colorRandomize(6, s.colorPaletteRandom("229,172,26\r\n229,156,26\r\n238,211,172\r\n253,230,191", d.GRAVEL)),grass: s.colorRandomize(2, s.colorPaletteRandom("218,116,3\r\n238,136,23\r\n228,126,13", d.GRASS)),water: s.colorRandomize(4,s.color(255, 166, 0, d.WATER)),},

                scene: {
                    init: function(n, r) {
                        function t() {
                            return Math.trunc(1024 * Math.random())
                        }
                        n.layeredTemplate().layer(10, !1, d.AIR).layer(10, !1, d.WALL).layerPerlin([{
                            factor: 120,
                            threshold: 0,
                            force: 80,
                            seed: t()
                        }, {
                            factor: 30,
                            threshold: 0,
                            force: 10,
                            seed: t()
                        }, {
                            factor: 5,
                            threshold: 0,
                            force: 2,
                            seed: t()
                        }], !0, d.WALL), n.graphics().fill(s.conditional(A.IS_STATIC, f.getBrush()));
                        const o = e.mapPositionFracWH(n, 100, 100);
                        n.graphics().drawRectangle(o.x(30), o.y(40), o.x(70), o.y(50), g.getBrush());
                        n.graphics().drawRectangle(o.x(10), o.y(60), o.x(40), o.y(70), H.getBrush());
                        n.graphics().drawRectangle(o.x(1), o.y(50), o.x(40), o.y(60), g.getBrush());
                        n.graphics().drawRectangle(o.x(70), o.y(50), o.x(100), o.y(60), h.getBrush());
                        n.graphics().drawRectangle(o.x(70), o.y(50), o.x(100), o.y(52), F.getBrush());
                        n.graphics().drawRectangle(o.x(68), o.y(20), o.x(69), o.y(21), U.getBrush());
                        n.graphics().drawRectangle(o.x(0), o.y(90), o.x(100), o.y(100), G.getBrush());
                   
                    }
                },
                primaryTool: BASIC_SHREDDED
            };
        return t.init(n, r, j)
    }, Object.defineProperty(n, "__esModule", {
        value: !1
    })
}));
